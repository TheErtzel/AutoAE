...
def getKey(key: str = 'f1'):
    ...
    if index != -1: return consts.KeyList[index]
    else: return 'f1'
...
def CropForeground(imageFile=None):
    if imageFile is None: return None

    originalImage = cv2.imread(imageFile)
    grayImage =  toGrayScale(originalImage.copy())

    (thresh, grayScaleImage) = cv2.threshold(grayImage, 127, 255, cv2.THRESH_BINARY)
...
def TakeScreenshot(file='', location = {'top': 0, 'left': 0, 'width': 1080, 'height': 1920}, crop = True):
    ...
    with mss.mss() as sct:
        ...
        output = "\\images\\tmp\\sct-{top}x{left}_{width}x{height}.png".format(**monitor)
        if file != '': output = f'{file}'
